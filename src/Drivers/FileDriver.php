...
use marcopgordillo\Press\Exceptions\FileDriverDirectoryNotFoundException;
    ...
    protected function validateSource()
    {
        if (!File::exists($this->config['path'])) {
            throw new FileDriverDirectoryNotFoundException(
                "Directory at '". $this->config['path'] . "' does not exists. Check the directory path in the config file."
            );
        }
    }
}
